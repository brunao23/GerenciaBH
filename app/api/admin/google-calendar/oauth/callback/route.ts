...
function redirectByFlow(
  ...
  return flow === "tenant"
    ? redirectToAgentPage(req, status, message)
    : redirectToUnits(req, status, message)
}

export async function GET(req: NextRequest) {
  let redirectFlow: "admin" | "tenant" = "admin"
  try {
    const url = new URL(req.url)
    const rawState = String(url.searchParams.get("state") || "").trim()
    let preParsedState: any = null
    if (rawState) {
      try {
        preParsedState = verifyAndParseState(rawState)
      } catch {
        preParsedState = null
      }
    }
    const flow: "admin" | "tenant" = preParsedState?.flow === "tenant" ? "tenant" : "admin"
    redirectFlow = flow

    const cookieStore = await cookies()
    const token = cookieStore.get("auth-token")?.value
    if (!token) {
      return redirectByFlow(req, flow, "error", "nao_autenticado")
    }

    const session = await verifyToken(token)
    if (!session) {
      return redirectByFlow(req, flow, "error", "acesso_negado")
    }

    const oauthError = String(url.searchParams.get("error") || "").trim()
    if (oauthError) {
      return redirectByFlow(req, flow, "error", oauthError)
    }

    const code = String(url.searchParams.get("code") || "").trim()
    const state = String(url.searchParams.get("state") || "").trim()
    if (!code || !state) {
      return redirectByFlow(req, flow, "error", "code_ou_state_ausente")
    }

    const statePayload = preParsedState || verifyAndParseState(state)
    const resolvedFlow: "admin" | "tenant" = statePayload?.flow === "tenant" ? "tenant" : flow
    redirectFlow = resolvedFlow
    const unitPrefix = String(statePayload?.unitPrefix || "").trim()
    if (!unitPrefix) {
      return redirectByFlow(req, resolvedFlow, "error", "state_unidade_invalida")
    }

    if (resolvedFlow === "admin") {
      if (!session.isAdmin) {
        return redirectByFlow(req, resolvedFlow, "error", "acesso_negado")
      }
    } else {
      const stateUnit = normalizeTenant(statePayload?.unitPrefix || statePayload?.tenant || "")
      const sessionUnit = normalizeTenant(session.unitPrefix || "")
      const canAccessTenant = Boolean(session.isAdmin || (stateUnit && sessionUnit && stateUnit === sessionUnit))
      if (!canAccessTenant) {
        return redirectByFlow(req, resolvedFlow, "error", "acesso_negado")
      }
    }
    ...
    const current =
      (await getNativeAgentConfigForTenant(unitPrefix)) ||
      ({
        enabled: false,
    ...
    if (!oauthClientId || !oauthClientSecret) {
      return redirectByFlow(req, resolvedFlow, "error", "google_oauth_client_id_ou_secret_ausente")
    }
    ...
    if (!tokenResponse.ok) {
      ...
      return redirectByFlow(req, resolvedFlow, "error", message)
    }
    ...
    if (!refreshToken) {
      return redirectByFlow(req, resolvedFlow, "error", "refresh_token_nao_retornoou")
    }
    ...
    await updateNativeAgentConfigForTenant(unitPrefix, nextConfig)
    return redirectByFlow(req, resolvedFlow, "connected", unitPrefix)
  } catch (error: any) {
    return redirectByFlow(req, redirectFlow, "error", error?.message || "falha_callback_oauth")
  }
}
