...
export class GroqService implements LLMService {
    ...
    private mergeUsage(base: LLMUsageMetrics | null, incoming: LLMUsageMetrics): LLMUsageMetrics {
        ...
        return {
            provider: incoming.provider || base.provider || "groq",
    ...
    private async requestGroq(payload: Record<string, any>): Promise<any> {
        if (!this.apiKey) {
        ...
        if (!response.ok) {
            const errorMsg = data?.error?.message || rawText || `Groq error: ${response.status}`;
            console.error(`[Groq] Request failed: status=${response.status} model=${this.model} error=${errorMsg}`);
    ...
    async decideNextTurn(input: {
        ...
        const data = await this.requestGroq(payload);
        const usage = this.extractUsageMetrics(data)
        const text = String(data?.choices?.[0]?.message?.content || "").trim();

        if (!text) {
            console.warn(`[Groq] Empty response model=${this.model}`);
            return {
                reply: "",
                actions: [{ type: "none" }],
                handoff: false,
    ...
    async decideNextTurnWithTools(input: {
        ...
        for (let step = 0; step < maxSteps; step++) {
            ...
            if (!responseMessage) {
                console.warn(`[Groq] No response message step=${step} model=${this.model}`);
                break;
            }
