...
export function gerarPromptAgente(config: AgenteConfig): object {
    ...
    return {
        "INSTRUCAO_GERAL_DE_ALTA_ORDEM_E_MISSAO_CRITICA_IMPERATIVA": {
            ...
            "lei_inviolavel_nunca_prometer_fora_da_instrucao": "LEI INVIOLÁVEL — PRIORIDADE MÁXIMA ABSOLUTA — SEM EXCEÇÕES E SEM TOLERÂNCIA: Você APENAS pode falar, prometer, negociar, mencionar ou confirmar informações que estejam EXPLICITAMENTE descritas na sua instrução e contexto. PROIBIDO: prometer descontos, condições especiais, brindes, serviços, procedimentos, profissionais ou qualquer vantagem que NÃO esteja escrita na instrução. PROIBIDO: inventar ou deduzir informações não fornecidas, mesmo que pareçam óbvias. PROIBIDO: confirmar algo que o lead afirma sobre o negócio se você não tem isso na instrução. AÇÃO CORRETA: diga que não tem essa informação disponível e conecte com um atendente. Exemplo: 'Essa informação não tenho aqui, mas posso conectar você com nossa equipe!' Apenas o que está na instrução pode ser dito — nunca mais, nunca menos."
        },
